...
module AsyncMatrixDocs
  module MarkdownExport
    ...
    def resolved_markdown(item, payload)
      ...
      Jekyll.logger.warn('async-matrix-docs', "Markdown export failed for #{relative_path}: #{e.message}")
      raw
    end
...
capture_markdown = lambda do |item, payload|
  next unless AsyncMatrixDocs::MarkdownExport.enabled?(item)

  raw = AsyncMatrixDocs::MarkdownExport.resolved_markdown(item, payload)
  item.data['_raw_markdown'] = AsyncMatrixDocs::MarkdownExport.with_title(raw, item.data['title'])
end
...
Jekyll::Hooks.register :site, :post_write do |site|
  AsyncMatrixDocs::MarkdownExport.write_files(site)
end
